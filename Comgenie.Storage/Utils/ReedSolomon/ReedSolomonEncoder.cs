...
namespace Comgenie.Storage.Utils.ReedSolomon
{
    ...
    */
    ...
    internal sealed class ReedSolomonEncoder
    {
        ...
        public byte[] EncodeEx(byte[] toEncode, int ecBytes, int offset, int length)
        {
            if (ecBytes == 0)
                throw new ArgumentException("No error correction bytes");

            //int dataBytes = length - ecBytes;

            //if (dataBytes <= 0)
            //    throw new ArgumentException("No data bytes provided");

            GenericGFPoly generator = BuildGenerator(ecBytes);
            int[] infoCoefficients = toEncode.Skip(offset).Take(length).Select(x => (int)x).ToArray();
